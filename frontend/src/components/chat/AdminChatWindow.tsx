...
const AdminChatWindow: React.FC<AdminChatWindowProps> = ({ messages, adminId }) => {
    ...
    >
      {messages.length === 0 ? (
        ...
        messages.map((msg, index) => (
          <div
            key={index}
            className={`mb-4 ${msg.sender_id.toString() === adminId ? 'text-right' : 'text-left'}`}
          >
            <div
              className={`inline-block p-3 rounded-lg shadow-soft ${
